...
def show_go_screen():
    ...
    draw_text(screen, "By: Josh Bothell", 16, WIDTH / 2, HEIGHT / 4 + 64)
    draw_text(screen, "Arrow keys move, Space to fire", 22,
              WIDTH / 2, HEIGHT / 2)
    draw_text(screen, "Press a key to begin", 18, WIDTH / 2, HEIGHT * 3 / 4)
    if high_score > 0:
        draw_text(screen, "High Score: " + str(high_score), 22, WIDTH / 2, HEIGHT * 5/6)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(FPS)
...
while running:
    ...
    # if the player dies and explosion finishes end game
    if player.lives == 0 and not death_explosion.alive():
       game_over = True

    # check for bullet collide with mob
    ...
    # check for player collide with powerup
